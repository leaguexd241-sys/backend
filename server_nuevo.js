// server.js - VERSIÓN COMPLETA CORREGIDA CON MEJORAS DE SEGURIDAD Y GAS PRICE CONFIGURABLE
...
// NUEVA VARIABLE: Gas price fijo en Gwei (si se define, se usará en lugar del cálculo automático)
const RELAY_GAS_PRICE_GWEI = process.env.RELAY_GAS_PRICE_GWEI 
  ? parseInt(process.env.RELAY_GAS_PRICE_GWEI, 10) 
  : null;
...
// Opciones: HSM, env vars, o cold storage rotation
const KEY_MANAGEMENT_TYPE = process.env.KEY_MANAGEMENT_TYPE || 'ENV_VARS';

// WALLET DEL RELAYER - Sistema seguro de rotación
let relayerWallet;
...
if (KEY_MANAGEMENT_TYPE === 'ENV_VARS') {
  ...
  if (RELAYER_PRIVATE_KEY && NODE_ENV === 'development') {
    // En desarrollo, solo mostrar parte de la clave
...
async function rotateRelayerKey() {
  try {
    if (process.env.KEY_ROTATION_ENABLED === 'true') {
      // En un sistema real, aquí se integraría con HSM o servicio de gestión de claves
      const newKey = ethers.Wallet.createRandom().privateKey;
      console.log('🔄 Rotando clave del relayer...');
      
      // Guardar en variable temporal (en producción usar HSM)
...
class MultiSigManager {
  ...
  async initialize() {
    // En producción, cargar desde configuración
  ...
  async requireTimeLock(transaction, delay = null) {
    ...
    // Registrar transacción con time-lock
    console.log(`⏰ Time-lock aplicado: ${waitTime} segundos`);
    
    // En implementación real, usaríamos un contrato de time-lock
...
const CONTRACTS = {
  ...
  // Reemplaza el contrato SIMPLE_MESSAGE_LOGGER con:
  SIMPLE_MESSAGE_LOGGER: {
    address: process.env.SIMPLE_MESSAGE_LOGGER_ADDRESS || '0x5b52665d600a48452a84C3B3E2ed02435b489ED9',
    name: 'SecureMessageLogger', // Cambia el nombre
    description: 'Contrato seguro para registrar mensajes en la blockchain',
    abi: loadContractABI('SecureMessageLogger') // Asegúrate de tener este ABI
  },
  
  MESSAGE_CONTRACT: {
    address: process.env.MESSAGE_CONTRACT_ADDRESS || '0x...',
    name: 'MessageContract',
    description: 'Contrato para enviar mensajes desde el juego',
    abi: loadContractABI('MessageContract')
  },
  
  ITEMS_CONTRACT: {
    address: process.env.ITEM_CONTRACT_ADDRESS || '0x18bf65B542CD207C8996234d500A63935ad940e8',
    name: 'ItemContract',
  ...
  REWARD_CONTRACT: {
    address: process.env.REWARD_CONTRACT_ADDRESS || '0x...',
    name: 'RewardContract',
    description: 'Contrato para recompensas del juego',
    abi: loadContractABI('RewardContract')
  }
};
  ...
  async getNextNonce() {
    ...
    return new Promise(async (resolve) => {
      ...
      const processQueue = async () => {
        ...
        try {
          // Actualizar nonce desde la red para evitar conflictos
...
const SECURITY_CONFIG = {
  ...
  // CORREGIDO: Remover IPs locales de la lista negra para desarrollo
  ...
  THRESHOLDS: {
    ...
    MIN_GAS_PRICE_MULTIPLIER: 1.2,
    
    // Protección contra Gas Drain - AUMENTAR PARA SOMNIA
    MAX_GAS_LIMIT: 10000000, // Aumentar a 10 millones
    MAX_GAS_PRICE_GWEI: 5000, // Aumentar a 5000 gwei (5 micro STT)
    MIN_VALUE_PER_GAS: 0.000001 // Valor mínimo por unidad de gas (STT)
  }
};
...
const contractWhitelistSchema = new mongoose.Schema({
  ...
  securityConfig: {
    ...
    maxGasLimit: { type: Number, default: 500000 }
  },
...
const ERROR_PASSWORD = '123456789000';
...
class RelayManager {
  ...
  console.log('🔄 Inicializando whitelist de contratos con configuración optimizada...');
  
  try {
    // Agregar contratos predefinidos a la whitelist CON CONFIGURACIÓN ESPECÍFICA
    for (const [key, contract] of Object.entries(CONTRACTS)) {
      console.log(`📝 Procesando contrato: ${key} - ${contract.name} - ${contract.address}`);
      ...
      if (contract.address && contract.address !== '0x...') {
        ...
        // Configuración por defecto para todos los contratos
        let securityConfig = {
          maxCallsPerHour: 100,
          maxCallsPerDay: 1000,
          requirePlayerOwnership: false,
          allowedFunctions: [], // Vacío por defecto = todas permitidas
          minGasPriceMultiplier: 1.2,
          maxGasLimit: 2000000 // 2 millones por defecto
        };
        
        // CONFIGURACIÓN ESPECIAL PARA SecureMessageLogger
        if (contract.name === 'SecureMessageLogger') {
          securityConfig = {
            maxCallsPerHour: 100,
            maxCallsPerDay: 1000,
            requirePlayerOwnership: false,
            allowedFunctions: ["logMessage", "getMessage", "messageCount"], // Funciones explícitas
            minGasPriceMultiplier: 5.0, // 500% para Somnia
            maxGasLimit: 10000000 // 10 millones
          };
          console.log(`🔧 Configuración especial para SecureMessageLogger`);
        }
        ...
        // CONFIGURACIÓN ESPECIAL PARA MESSAGE_CONTRACT
        if (contract.name === 'ItemContract') {
          securityConfig = {
            maxCallsPerHour: 50,
            maxCallsPerDay: 500,
            ...
            'deleteTipoAndInvoices',
            'listTipos',
            'getInvoice',
            'getInvoiceByManualId',
            'getUserInvoiceCounts',
            'getUserTypes',
            'getTipoStats',
            'getTotalActiveInvoices'
          ],
        ...
        if (!existing) {
          console.log(`➕ Agregando ${contract.name} a whitelist con configuración optimizada...`);
          ...
          // Si ya existe, actualizar la configuración si es necesario
          console.log(`🔄 Actualizando configuración de ${contract.name}...`);
    ...
    // Verificar que SecureMessageLogger tiene la configuración correcta
    const secureLogger = await ContractWhitelist.findOne({
      contractName: 'SecureMessageLogger'
    });
    
    if (secureLogger) {
      console.log(`🔍 Verificando SecureMessageLogger:`);
      console.log(`   - Max Gas Limit: ${secureLogger.securityConfig?.maxGasLimit}`);
      console.log(`   - Allowed Functions: ${secureLogger.securityConfig?.allowedFunctions?.length || 0} funciones`);
      console.log(`   - Gas Price Multiplier: ${secureLogger.securityConfig?.minGasPriceMultiplier}`);
    }
    
    console.log('✅ Whitelist inicializada y optimizada para Somnia Testnet');
  } catch (error) {
    console.error('❌ Error inicializando whitelist:', error);
  }
}
  ...
  const {
    ...
    playerName,
    contractAddress,
    contractName, // AÑADIDO: recibir contractName
    functionName,
    parameters,
    ip,
    userAgent,
  ...
  // Crear registro inicial - AÑADIR contractName
  const relayTx = new RelayedTransaction({
    transactionId,
    internalId,
    playerAddress,
    playerName,
    contractName, // AÑADIDO
  ...
  await relayTx.save();
    ...
    try {
      ...
      try {
        ...
        gasLimit = 200000n; // Valor por defecto seguro
      }
      
      // 6. Obtener gas price
      let gasPrice;

      if (RELAY_GAS_PRICE_GWEI !== null) {
        // Usar el valor fijo configurado (convertir de gwei a wei)
        gasPrice = ethers.parseUnits(RELAY_GAS_PRICE_GWEI.toString(), 'gwei');
        console.log(`⚙️ Usando gas price fijo: ${RELAY_GAS_PRICE_GWEI} gwei`);
      } else {
        // Lógica original: obtener precio de la red y aplicar multiplicador
        const feeData = await provider.getFeeData();
        gasPrice = feeData.gasPrice || await provider.getGasPrice();

        // Aplicar multiplicador de seguridad
        const multiplier = whitelisted.securityConfig.minGasPriceMultiplier || 1.2;
    ...
    try {
      ...
      if (whitelisted) {
        ...
        if (whitelisted.securityConfig.allowedFunctions.length > 0 &&
          ...
          return {
            allowed: false,
            reason: `Function ${functionName} not allowed for this contract`
          };
        }
      }
  ...
  try {
    ...
    if (receipt) {
      ...
      if (receipt.gasUsed && receipt.effectiveGasPrice) {
        ...
        // Actualizar estadísticas con BigInt
        if (receipt.status === 1) {
      ...
      console.log(`✅ Transacción ${txHash} ${receipt.status === 1 ? 'confirmada' : 'revertida'} en bloque ${receipt.blockNumber}`);
    }
  } catch (error) {
    console.error(`❌ Error esperando confirmación para ${txHash}:`, error);
    
    // Marcar como fallida después de varios intentos
    relayTx.status = 'failed';
    ...
    try {
      ...
      if (!gasLimit || !gasPrice) {
        ...
        if (relayTx.functionName === 'logMessage') {
          ...
          gasLimit = 2000000n; // 2 millones de gas para STT
          ...
          gasLimit = 3000000n; // 3 millones de gas
          console.log(`   - Gas limit por defecto para STT: ${gasLimit}`);
        }
        
        // 2. Configurar gas price
        // Usar valor fijo si está definido, sino calcular
        if (RELAY_GAS_PRICE_GWEI !== null) {
          gasPrice = ethers.parseUnits(RELAY_GAS_PRICE_GWEI.toString(), 'gwei');
          console.log(`⚙️ Usando gas price fijo en reintento: ${RELAY_GAS_PRICE_GWEI} gwei`);
        } else {
          try {
            const feeData = await provider.getFeeData();
            gasPrice = feeData.gasPrice || await provider.getGasPrice();
            ...
            // Aumentar significativamente el gas price para STT (100% más)
            gasPrice = (gasPrice * 200n) / 100n; // +100% para asegurar

            // Mínimo de 100 gwei para STT (red Somnia)
            const minGasPrice = ethers.parseUnits("100", "gwei");
            if (gasPrice < minGasPrice) {
              gasPrice = minGasPrice;
              console.log(`   - Gas price aumentado a mínimo para STT: ${ethers.formatUnits(gasPrice, 'gwei')} gwei`);
            } else {
              console.log(`   - Gas price aumentado (100%): ${ethers.formatUnits(gasPrice, 'gwei')} gwei`);
            }

          } catch (gasError) {
            console.warn(`⚠️  Error obteniendo gas price, usando valor fijo:`, gasError.message);
            // Valor fijo alto para STT: 200 gwei
            gasPrice = ethers.parseUnits("200", "gwei");
            console.log(`   - Gas price fijo para STT: ${ethers.formatUnits(gasPrice, 'gwei')} gwei`);
          }
        }
      }
      ...
      // Log detallado para SimpleMessageLogger
      if (relayTx.functionName === 'logMessage') {
        ...
        // Asegurar que _userNonce sea string para uint256
      ...
      // 10. Emitir evento si está disponible
      if (global.io) {
      ...
      // 11. Iniciar seguimiento de confirmación
      ...
      // Emitir evento de error si está disponible
...
const corsOptions = {
  origin: function(origin, callback) {
    // Permitir sin origen (Postman, mobile apps, scripts, etc.)
    if (!origin) {
      return callback(null, true);
    }
    
    // PERMITIR TODO EN DESARROLLO (localhost y 127.0.0.1)
    if (NODE_ENV === 'development') {
...
app.use((req, res, next) => {
  if (NODE_ENV === 'development') {
    ...
    // Headers para permitir cookies cross-origin en desarrollo
...
const createCustomRateLimiter = (windowMs, max, message, skipDevelopment = false) => {
  return rateLimit({
    ...
    legacyHeaders: false,
    // CORREGIDO: Removido skipKeyGeneratorIpFallback que causaba error
    keyGenerator: (req) => {
      // Usar IP del cliente o IP normal
      const ip = req.clientIp || req.ip;
      return ip || 'unknown';
    },
    skip: (req) => {
      // En desarrollo, saltar rate limit para IPs locales
...
function authMiddleware(req, res, next) {
  ...
  console.log('📋 Headers recibidos:', {
    cookies: req.cookies ? Object.keys(req.cookies) : 'No hay cookies',
...
// --- RUTAS DE API COMPLETAS CORREGIDAS ---

// server.js - Agrega esta ruta temporalmente
app.get('/api/debug/contracts', async (req, res) => {
  try {
    const dbContracts = await ContractWhitelist.find({ enabled: true }).lean();
    const predefined = Object.values(CONTRACTS);
    
    res.json({
      dbContracts: dbContracts.map(c => ({
        name: c.contractName,
        address: c.contractAddress,
        enabled: c.enabled
      })),
      predefined: predefined.map(c => ({
        name: c.name,
        address: c.address,
...
app.get('/api/health', async (req, res) => {
  try {
    const [mongoStatus, relayStats, relayerBalance] = await Promise.all([
      mongoose.connection.readyState === 1,
      relayManager.getStats(),
      checkRelayerBalance()
    ]);
    ...
    res.json({ 
      ok: true, 
      timestamp: Date.now(), 
      service: 'Grassland Forest Backend',
      version: '5.0.0',
      features: {
        relay: relayerWallet ? 'ACTIVADO' : 'DESACTIVADO',
        security: 'ACTIVADO',
        contracts: Object.keys(CONTRACTS).length,
        environment: NODE_ENV,
        keyManagement: KEY_MANAGEMENT_TYPE,
        gasProtection: 'ACTIVADO',
        gasPriceFixed: RELAY_GAS_PRICE_GWEI !== null ? `${RELAY_GAS_PRICE_GWEI} gwei` : 'automático'
      },
      database: {
      ...
      relay: relayStats,
      balance: relayerBalance ? ethers.formatEther(relayerBalance) : '0',
...
// CORREGIDO: Endpoint de DEBUG para verificar nonce
app.get('/api/auth/debug-nonce/:address', async (req, res) => {
  try {
    const address = req.params.address.toLowerCase();
    console.log(`🔍 Debug nonce para: ${address}`);
    
    const player = await PlayerAuth.findOne({ address }).lean();
    ...
    const now = new Date();
    const nonceAge = player.nonceTimestamp ? 
      (now.getTime() - new Date(player.nonceTimestamp).getTime()) : 
      null;
    
    const isExpired = player.nonceTimestamp ? 
      (nonceAge > 10 * 60 * 1000) : 
      true;
    ...
    return res.json({
      ...
      nonce: player.nonce,
      nonceTimestamp: player.nonceTimestamp,
      nonceAge: nonceAge ? `${Math.floor(nonceAge/1000)} segundos` : 'N/A',
      isExpired,
      hasNonce: !!player.nonce,
      playerName: player.playerName,
      lastLogin: player.lastLogin,
      createdAt: player.createdAt,
      loginAttempts: player.loginAttempts,
      loginBlockedUntil: player.loginBlockedUntil,
      // Verificar si está bloqueado ahora
      isCurrentlyBlocked: player.loginBlockedUntil ? 
        (new Date(player.loginBlockedUntil) > now) : 
        false
    });
  } catch (error) {
  ...
  async (req, res) => {
    try {
      const contracts = await ContractWhitelist.find({ enabled: true })
        .select('contractAddress contractName description securityConfig stats')
        .lean();
...
app.post('/api/save/:playerName',
  ...
  async (req, res) => {
    ...
    const address = req.user.address.toLowerCase();
    const { inventory, chest, missionsData } = req.body;
...
server.listen(PORT, HOST, () => {
  ...
  if (RELAY_GAS_PRICE_GWEI !== null) {
    console.log(`⚙️  Gas price fijo: ${RELAY_GAS_PRICE_GWEI} gwei`);
  } else {
    console.log(`⚙️  Gas price: automático (con multiplicador)`);
  }
  console.log(`=================================`);
});
