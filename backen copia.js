...
const FRONTEND_ORIGIN = process.env.FRONTEND_ORIGIN || 'http://192.168.100.11:5501';
...
// CORRECCIÓN CRÍTICA: Función de validación de nonce corregida
function validateNonce(userNonce, storedNonce) {
  try {
    ...
    // CORRECCIÓN: El nonce enviado debe ser exactamente igual al almacenado
    if (u !== s) {
...
// ─────────────────────────────────────────────────────────────────
// SOCKET.IO HANDLERS CON SOPORTE PARA SALAS (ROOMS)
// ─────────────────────────────────────────────────────────────────
...
const rooms = {
  'game': {},    // Jugadores en GameScene
  'tienda': {}   // Jugadores en tiendajuego
};
...
io.on("connection", (socket) => {
  ...
  socket.on("joinRoom", (data) => {
    ...
    console.log(socket.playerData.room);
    console.log(room);
    if (socket.playerData.room && socket.playerData.room !== room) {
      ...
      if (rooms[socket.playerData.room]) {
        ...
        // Notificar solo si hay otros jugadores
  ...
  socket.on("playerMove", (data) => {
    ...
    // Actualizar con todos los datos incluyendo si está moviéndose
    rooms[room][socket.id] = {
      ...rooms[room][socket.id],
      ...data,
      isMoving: data.isMoving || false, // Nueva propiedad
      lastUpdate: Date.now()
    };
...
socket.on('chatMessage', (payload) => {
  try {
    ...
    const message = {
      ...
      room: room // Agregar información de sala
    };
...
class CropController {
  constructor(io) {
    ...
    // Limpiar historial al iniciar el backend
    ...
    this.cropTypes = {
      Semillax: {
        ...
        rewards: {
          item: 'Bolsa_de_cristales',
          quantity: 2,
          progress_reward: 'Bolsa_de_cristales',
          progress_quantity: 3,
          deadReward: 'Bolsa_de_cristales',
          deadQuantity: 1
        }
      },
      SemillaRapida: {
        ...
        rewards: {
          item: 'Bolsa_de_cristales',
          quantity: 1,
          progress_reward: 'palo_de_madera',
          progress_quantity: 2,
          deadReward: 'Cristal_roto',
          deadQuantity: 1
        }
      },
      ArbolGrande: {
        ...
        rewards: {
          item: 'Madera_pinos',
          quantity: 5,
          progress_reward: 'madera_pequena',
          progress_quantity: 3,
          deadReward: 'Madera_podrida',
          deadQuantity: 2
        }
      },
      SemillaAvanzada: {
        id: 'SemillaAvanzada',
        name: 'Semilla Avanzada',
        type: 'semilla',
        growthStages: 4,
        growthTime: 180,
        waterRequired: true,
        ...
        rewards: {
          item: 'Bolsa_de_cristales_grande',
          quantity: 5,
          progress_reward: 'cristal_pequeno',
          progress_quantity: 2,
          deadReward: 'Cristal_roto',
          deadQuantity: 2
        }
  ...
  async plantSeed(userId, plotId, seedType, userStats, successChance) {
    try {
      ...
      // Ajustar probabilidad: si es 100%, restar 5% para dejarlo en 95%
      let adjustedChance = parseFloat(successChance);
      ...
      console.log(`🌱 ${userId} plantó ${seedType} en ${plotId} - Posibilidad ajustada: ${adjustedChance}%`);
  ...
  async harvestCrop(userId, plotId) {
    try {
      ...
      // ELIMINAR EL CULTIVO DE USER CROPS EN LUGAR DE MARCAR COMO HARVESTED
  ...
  async cutCrop(userId, plotId) {
    try {
      ...
      if (crop.isDead) {
        // Usar recompensa muerta si el árbol está muerto
        rewards = {
        ...
        // Usar recompensa de progreso si el árbol está vivo pero no completado
        rewards = {
        ...
        // Este caso no debería ocurrir porque los árboles completados se cosechan, no se cortan
...
api.post('/save/:playerName', 
  ...
  [
    ...
    body('chest').isArray(),
    body('missionsData').optional().isObject() // NUEVO: datos de misiones
  ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    try {
      ...
      // NUEVO: Guardar datos de misiones si vienen en la petición
...
api.get('/load/:playerName', 
  ...
  async (req, res) => {
    ...
    try {
      ...
      // NUEVO: Cargar datos de misiones
      ...
      res.json({ 
        ...
        missionsData: missionsData.toObject() // NUEVO: incluir datos de misiones
      });
    } catch (e) {
...
// ─────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────
// RUTAS ESPECÍFICAS PARA MISSIONS PLAYER - CORREGIDAS
// ─────────────────────────────────────────────────────────────────

// Obtener datos de misiones específicos
...
// Actualizar datos de misiones específicos
...
api.post('/listingsx/:playerName',
  ...
  async (req, res) => {
    ...
    const playerName = req.params.playerName; // Obtener de los parámetros
    const { inventoryId, name, type, qty, price, imageUrl } = req.body;

    // Verificar que el playerName del parámetro coincide con el usuario autenticado
    const user = await User.findOne({ address: req.user.address });
    if (!user) {
      return res.status(404).json({ error: 'Usuario no encontrado' });
    }

    // Si el usuario no tiene playerName en User, asignarlo
    if (!user.playerName) {
      user.playerName = playerName;
      await user.save();
    } else if (user.playerName !== playerName) {
    ...
    try {
      const newListing = new Listing({
        owner: playerName, // Usar playerName del parámetro
        inventoryId,
        name,
        type,
        qty,
